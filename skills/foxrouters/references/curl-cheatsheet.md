...
## Combos (admin only, v1.4.0)

```bash
# List
curl -s $GW/api/combos -H "Authorization: Bearer $KEY"

# Create Fallback combo — tries models[0], retries next on 5xx
curl -s -X POST $GW/api/combos \
  -H "Authorization: Bearer $KEY" -H "content-type: application/json" \
...
# Create Round Robin combo — rotates across requests (atomic Redis INCR)
curl -s -X POST $GW/api/combos \
  -H "Authorization: Bearer $KEY" -H "content-type: application/json" \
...
# Fetch one
curl -s $GW/api/combos/smart-fallback -H "Authorization: Bearer $KEY"

# Delete (also removes combo:counter:<name>)
curl -s -X DELETE $GW/api/combos/smart-fallback -H "Authorization: Bearer $KEY"

# Use — client calls combo/<name> like any model
curl -s $GW/v1/chat/completions -H "Authorization: Bearer $KEY" -H "content-type: application/json" \
  -d '{"model":"combo/smart-fallback","messages":[{"role":"user","content":"hi"}]}'

# Verify combos appear in /v1/models
curl -s $GW/v1/models -H "Authorization: Bearer $KEY" | jq '.data[] | select(.id | startswith("combo/"))'
```

Backing store: Redis HASH `combos` (field=name, value=JSON) + `combo:counter:<name>` STRING (round-robin INCR).
Fallback retry is non-streaming only; SSE clients use `models[0]` head-of-list.
